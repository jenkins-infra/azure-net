...
resource "azurerm_dns_a_record" "vpn" {
  name                = local.vpn.private_shorthostname
...
resource "azurerm_linux_virtual_machine" "vpn" {
  ...
  user_data = base64encode(templatefile("./.shared-tools/terraform/cloudinit.tftpl", { hostname = join(".", [local.vpn.private_shorthostname, data.azurerm_dns_zone.jenkinsio.name]) }))
  # Force VM recreation when the VPN URL change
  computer_name = replace(join(".", [local.vpn.private_shorthostname, data.azurerm_dns_zone.jenkinsio.name]), ".", "-")
